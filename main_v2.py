...
def update_image(
        *args):  # die Funktion wird immer aufgerufen, wenn sich der Wert, der Combobox ändert. das "*args" ist ein Platzhalter
    # für beliebige Argumente die wir theoretisch übergeben können. Das brauchen wir für die "trace" Methode
    global img  # Definiert innerhalb der Funktion eine global abrufbare Variable
    auswahl = würfel_var.get()  # Holt den aktuellen Wert von "würfel_var" und speichert diesen in einer Variable
    if auswahl == würfel_typen[0]:
        img = ImageTk.PhotoImage(Image.open("W4.png").resize(
            (50, 50)))  # Ich muss im Zweifel den relativen Pfad angeben ;) Mit der Variable habe ich das BIld geladen.
...
würfel_var.trace('w',
                 update_image)  # Mit der "trace" Methode hat man die Möglichkeit Funktionen aufzurufen, sobald sich der Wert
# einer TKinter Variable ändert. Das "w" steht für "wrtie" was bedeutet, dass die Funktion aufgerufen wird, wenn der Wert der
# Variable geschrieben, also geändert wird.
...
def aktueller_würfel():
    if würfel_anzahl_combobox.current() == 0:  # Hier greift die "if" Abfrage vereinfacht auf den Index der Combobox bzw. der Liste von der Combobox zu
        ...
        anzeige_würfelaugen_ergebnis_1.config(
            text=würfel_ergebnis)  # Ändert den Text des Textlabels "anzeige_würfelaugen_ergebnis", auf den Inhalt der Variable "würfel_ergebnis"
...
# Mit dem Befehl "image=img" weise dem Button das Bild zu. Mit dem Befehl "cursor="hand2" verändert man den Mauszeiger im
# Bereich des Buttons
...
anzeige_würfelaugen_ergebnis_1 = ttk.Label(würfelaugen_ergebnis_label_frame, background="#BDBDBD",
                                           text="Hier kommt dein Würfelergebnis!", width=30)
anzeige_würfelaugen_ergebnis_1.pack(padx="5", pady="5")
anzeige_würfelaugen_ergebnis_2 = ttk.Label(würfelaugen_ergebnis_label_frame, background="#BDBDBD",
                                           text="Ändere deine Einstellung!", width=30)
anzeige_würfelaugen_ergebnis_2.pack(padx="5", pady="5")
anzeige_würfelaugen_ergebnis_3 = ttk.Label(würfelaugen_ergebnis_label_frame, background="#BDBDBD",
                                           text="Ändere deine EInstellung!", width=30)
